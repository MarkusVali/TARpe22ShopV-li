...
namespace TARpe22ShopVäli.ApplicationServices.Services
{
    public class FilesServices : IFilesServices
    {
        ...
        public FilesServices(TARpe22ShopVäliContext context, IHostingEnvironment webHost)
        {
            _context = context;
            _webHost = webHost;
        }
        ...
        public void CarUploadFilesToDatabase(CarDto dto, Car domain)
        {
            if (dto.Files != null && dto.Files.Count > 0)
            {
                foreach (var photo in dto.Files)
                {
                    using (var target = new MemoryStream())
                    {
                        ...
                        _context.CarFilesToDatabase.Add(files);
                    }
                }
            }
        }
        ...
        public async Task<List<FileToDatabase>> CarRemoveImagesFromDatabase(CarFileToDatabaseDto[] dtos)
        {
            foreach (var dto in dtos)
            {
                var image = await _context.FilesToDatabase
